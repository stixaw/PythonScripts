...
stable = os.environ['Stable_PL'].lower()
spVersion = ''
plNameVersion = ''
pl_tempsrc = r'\\ali-netapp1.linus.sen.symantec.com\Build\Lindon\%s\SMP\PL' % (Release)
pl_template = 'platform.pl.xml'
stable_pl_tempsrc = r'\\ali-netapp1.linus.sen.symantec.com\Build\Lindon\%s\SMP\Stable_PL' % (Release)
stable_pl_template = 'platform_stable.pl.xml'
...
def grabTemplate(top):
	global pl_tempsrc, pl_template, pl_tempdst
...
def grabStableTemplate(top):
	global stable_pl_tempsrc, stable_pl_template, pl_tempdst
	#pl_tempdst = top + r'\CM\PLXML'
	#check for template in destination if its there remove it.
	if os.path.exists(pl_tempdst + '\\' + stable_pl_template):
		os.remove(pl_tempdst + '\\' + stable_pl_template)
	#copy new copy of template to local directory
	#shutil.copy(pl_tempsrc + '\\' + pl_template, pl_tempdst)
	#new copy of CBP template to local directory
	shutil.copy(stable_pl_tempsrc + '\\' + stable_pl_template, pl_tempdst)
...
def callModifyPL(top):
	...
	cmd = (top + r'\CM\Scripts\%s\CBPITMS_SP2.bat ' % (Release)) + PL_DIR + ' ' + PLVersion + ' ' +  top + ' ' + buildnum + ' ' + pl_template
	print "Calling Modifypl with this command: ", cmd
	os.system(cmd)
	#make copy of lindon unc pl:
	shutil.copy(pl_tempdst + '\\' + pl_template, pl_tempdst + '\\lindon_itms_' + plNameVersion + buildnum + '.pl.xml')
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\LINUSPLXML.txt", "w")
	text_file.write('lindon_itms_' + plNameVersion + buildnum + '.pl.xml')
	text_file.close()

def callStableModifyPL(top):
	global PL_DIR, repo, repo_relative, PLVersion, buildnum, stable_pl_template, pl_tempdst, plNameVersion, Release
	#call modify batch file with no repository change will be lindon UNC from PL_DIR (need to rem out repository call in batch file
	cmd = (top + r'\CM\Scripts\%s\CBPITMS_SP2.bat ' % (Release)) + PL_DIR + ' ' + PLVersion + ' ' +  top + ' ' + buildnum + ' ' + stable_pl_template
	print "Calling Modifypl with this command: ", cmd
	os.system(cmd)
	#make copy of lindon unc pl:
	shutil.copy(pl_tempdst + '\\' + stable_pl_template, pl_tempdst + '\\lindon_itms_' + plNameVersion + buildnum + '_stable.pl.xml')
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\LINUSSTABLEPLXML.txt", "w")
	text_file.write('lindon_itms_' + plNameVersion + buildnum + '_stable.pl.xml')
	text_file.close()

def callModifyRelativ(top):
	global PL_DIR, repo_relative, PLVersion, pl_template, pl_tempdst, plNameVersion, Release, buildnum
	...
	cmd_relative1 = ('cscript ' + top + r'\CM\Scripts\ReplaceMacro.vbs ' + '"' + pl_tempdst + '\\relative_itms_' + plNameVersion + PLVersion + '.pl.xml" ' + '"' + pl_tempdst + '\\relative_itms_' + plNameVersion + buildnum + '.pl.xml" ' + (r'"\\\\ali-netapp1.linus.sen.symantec.com\\Build\\Lindon\\%s" ' % (Release)) + '".."')
	print "Calling Modifypl with this command: ", cmd_relative1
	os.system(cmd_relative1)
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\RelativePLXML.txt", "w")
	text_file.write('relative_itms_' + plNameVersion + buildnum + '.pl.xml')
	text_file.close()

def callStableModifyRelativ(top):
	global PL_DIR, repo_relative, PLVersion, stable_pl_template, pl_tempdst, plNameVersion, Release, buildnum
	#call modify batch file with linus unc path for NewRepository
	#cmd_relative = (top + r'\CM\PLXML\ModifyPL.exe -pl:"' + top + r'\CM\PLXML\relative_itms_' + plNameVersion + PLVersion + '.pl.xml' + '" -action:PackagesChangeRepository "-OldRepository:%s" "-NewRepository:%s"'% (PL_DIR, repo_relative))
	cmd_relative = ('cscript ' + top + r'\CM\Scripts\ReplaceMacro.vbs ' + '"' + pl_tempdst + '\\' + stable_pl_template + '" ' + '"' + pl_tempdst + '\\relative_itms_' + plNameVersion + PLVersion + '_stable.pl.xml" ' + (r'"\\\\ali-netapp1.linus.sen.symantec.com\\build\\lindon\\%s" ' % (Release).lower()) + '".."')
	print "Calling Modifypl with this command: ", cmd_relative
	os.system(cmd_relative)
	cmd_relative1 = ('cscript ' + top + r'\CM\Scripts\ReplaceMacro.vbs ' + '"' + pl_tempdst + '\\relative_itms_' + plNameVersion + PLVersion + '_stable.pl.xml" ' + '"' + pl_tempdst + '\\relative_itms_' + plNameVersion + buildnum + '_stable.pl.xml" ' + (r'"\\\\ali-netapp1.linus.sen.symantec.com\\Build\\Lindon\\%s" ' % (Release)) + '".."')
	print "Calling Modifypl with this command: ", cmd_relative1
	os.system(cmd_relative1)
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\RelativeStablePLXML.txt", "w")
	text_file.write('relative_itms_' + plNameVersion + buildnum + '_stable.pl.xml')
	text_file.close()

def callModifyEstoniaUNC(top):
	global PL_DIR, repo, repo_relative, estonia_unc, PLVersion, pl_template, pl_tempdst, plNameVersion, buildnum
	#call modify batch file with linus unc path for NewRepository
	...
	cmd_estonia = ('cscript ' + top + r'\CM\Scripts\ReplaceMacro.vbs ' + '"' + pl_tempdst + '\\' + pl_template + '" ' + '"' + pl_tempdst + '\\estonia_itms_' + plNameVersion + buildnum + '.pl.xml" ' + r'"\\\\ali-netapp1.linus.sen.symantec.com" ' + r'"\\eta-netapp1.tales.sen.symantec.com"')
	print "Calling Modifypl with this command: ", cmd_estonia
	os.system(cmd_estonia)
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\TALESPLXML.txt", "w")
	text_file.write('estonia_itms_' + plNameVersion + buildnum + '.pl.xml')
	text_file.close()

def callStableModifyEstoniaUNC(top):
	global PL_DIR, repo, repo_relative, estonia_unc, PLVersion, stable_pl_template, pl_tempdst, plNameVersion
	#call modify batch file with linus unc path for NewRepository
	#cmd_estonia = (top + r'\CM\PLXML\ModifyPL.exe -pl:"' + top + r'\CM\PLXML\estonia_itms_' + plNameVersion + PLVersion + '.pl.xml' + '" -action:PackagesChangeRepository "-OldRepository:%s" "-NewRepository:%s"'% (PL_DIR, estonia_unc))
	cmd_estonia = ('cscript ' + top + r'\CM\Scripts\ReplaceMacro.vbs ' + '"' + pl_tempdst + '\\' + stable_pl_template + '" ' + '"' + pl_tempdst + '\\estonia_itms_' + plNameVersion + buildnum + '_stable.pl.xml" ' + r'"\\\\ali-netapp1.linus.sen.symantec.com" ' + r'"\\eta-netapp1.tales.sen.symantec.com"')
	print "Calling Modifypl with this command: ", cmd_estonia
	os.system(cmd_estonia)
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\TALESSTABLEPLXML.txt", "w")
	text_file.write('estonia_itms_' + plNameVersion + buildnum + '_stable.pl.xml')
	text_file.close()
...
def callModifyPuneUNC(top):
	...
	cmd_pune = ('cscript ' + top + r'\CM\Scripts\ReplaceMacro.vbs ' + '"' + pl_tempdst + '\\' + pl_template + '" ' + '"' + pl_tempdst + '\\pune_itms_' + plNameVersion + buildnum + '.pl.xml" ' + r'"\\\\ali-netapp1.linus.sen.symantec.com" ' + r'"\\pun-netapp1.punin.sen.symantec.com"')
	print "Calling Modifypl with this command: ", cmd_pune
	os.system(cmd_pune)
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\PUNINPLXML.txt", "w")
	text_file.write('pune_itms_' + plNameVersion + buildnum + '.pl.xml')
	text_file.close()

def callStableModifyPuneUNC(top):
	global PL_DIR, repo, repo_relative, pune_unc, PLVersion, stable_pl_template, pl_tempdst, plNameVersion
	#call modify batch file with linus unc path for NewRepository
	#cmd_pune = (top + r'\CM\PLXML\ModifyPL.exe -pl:"' + top + r'\CM\PLXML\pune_itms_' + plNameVersion + PLVersion + '.pl.xml' + '" -action:PackagesChangeRepository "-OldRepository:%s" "-NewRepository:%s"'% (PL_DIR, pune_unc))
	cmd_pune = ('cscript ' + top + r'\CM\Scripts\ReplaceMacro.vbs ' + '"' + pl_tempdst + '\\' + stable_pl_template + '" ' + '"' + pl_tempdst + '\\pune_itms_' + plNameVersion + buildnum + '_stable.pl.xml" ' + r'"\\\\ali-netapp1.linus.sen.symantec.com" ' + r'"\\pun-netapp1.punin.sen.symantec.com"')
	print "Calling Modifypl with this command: ", cmd_pune
	os.system(cmd_pune)
	#write out PL name to text files for email
	path = os.environ['WORKSPACE']
	text_file=open(path + "\\PUNINSTABLEPLXML.txt", "w")
	text_file.write('pune_itms_' + plNameVersion + buildnum + '_stable.pl.xml')
	text_file.close()
...
def copyFinalPL(top):
	...
	global pl_tempdst, pl_template, pl_tempsrc, stable_pl_template, Release
	ITMSPL_DIR = r'\\ali-netapp1.linus.sen.symantec.com\Build\Lindon\%s\PL' % (Release)
	LATESTPL_DIR = r'\\ali-netapp1.linus.sen.symantec.com\Build\Lindon\%s\PL\Latest' % (Release)
	pl_tempdst = top + r'\CM\PLXML'
	path = top
	...
	for xml in glob.glob(ITMSPL_DIR + '\\*_Administrator_ITMSCBPLWIN.pl.xml'):
		os.remove(xml)
	for xml in glob.glob(ITMSPL_DIR + '\\relative_itms_7_5_7.5*.pl.xml'):
		os.remove(xml)
	if os.path.exists(ITMSPL_DIR + '\\' + pl_template):
		os.remove(ITMSPL_DIR + '\\' + pl_template)
	if os.path.exists(ITMSPL_DIR + '\\' + stable_pl_template):
		os.remove(ITMSPL_DIR + '\\' + stable_pl_template)
	# copy and rename to latest directory
	shutil.copy(pl_tempdst + '\\' + 'lindon_itms_' + plNameVersion + buildnum + '.pl.xml', LATESTPL_DIR + '\\' + 'lindon_itms_' + plNameVersion + 'latest.pl.xml')
	shutil.copy(pl_tempdst + '\\' + 'estonia_itms_' + plNameVersion + buildnum + '.pl.xml', LATESTPL_DIR + '\\' + 'estonia_itms_' + plNameVersion + 'latest.pl.xml')
	shutil.copy(pl_tempdst + '\\' + 'pune_itms_' + plNameVersion + buildnum + '.pl.xml', LATESTPL_DIR + '\\' + 'pune_itms_' + plNameVersion + 'latest.pl.xml')


def copyPlatformPl():
	global pl_tempdst, pl_template
	print "Copying platform.pl.xml created from first run to ./pl/platform.pl.xml"
...
if __name__ == '__main__':
	try:
		...
		if stable == 'true':
			...
			print "Daily pl.xml created now starting on Stable Pl.xml process"
			grabStableTemplate(os.environ['WORKSPACE'])
			callStableModifyPL(os.environ['WORKSPACE'])
			callStableModifyRelativ(os.environ['WORKSPACE'])
			callStableModifyEstoniaUNC(os.environ['WORKSPACE'])
			callStableModifyPuneUNC(os.environ['WORKSPACE'])
		else:
			GetTime()
			GetplNameVersion()
			print plNameVersion
