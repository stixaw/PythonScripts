...
def getDate():
	...
	expDate =xDate - timedelta(days=31)
	print "Expiration date for comparison = ", expDate
	return expDate, xDate

def compareCBPFiles():
	global xDate, releaseName, expDate
	dirPath = r'\\ali-netapp1.linus.sen.symantec.com\Build\Lindon\%s\SMP' % (releaseName)
	#dirPath = r'\\ali-netapp1.linus.sen.symantec.com\Build\Lindon\pcanywhere\Builds\VegaRelease_Box'
	#get the date from subdirectories and root directories
	for root, dirs, files in os.walk(dirPath):
		for name in dirs:
			folder = os.path.join(root, name)
			folderdate =  date.fromtimestamp(os.path.getmtime(folder))
