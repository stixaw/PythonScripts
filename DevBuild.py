...
#calling build.py to run the no label manual build
  buildcommand = ('python ' + top + '\\build\\scripts\\build.py ' + '--top=' + top + ' --version=7.1. --buildnum=%s --nosign > Devbuid.log' % (rnum))
...
def CopyMSI():
  ...
  if os.path.exists(X64_DD):
    shutil.rmtree(X64_DD)

  if os.path.exists(X86_DD):
    shutil.rmtree(X86_DD)

  if not os.path.exists(X64_DD):
    os.makedirs(X64_DD)

  if not os.path.exists(X86_DD):
    os.makedirs(X86_DD)

  for MSI in X86_MSI:
    print "Copying x86 MSI"
    shutil.copy(MSI, X86_DD)

  for MSI in X64_MSI:
    print "Copying x64 MSI"
    shutil.copy(MSI, X64_DD)
